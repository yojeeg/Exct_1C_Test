...
Процедура СформироватьПечатнуюФорму(Команда)
	...
	Печать("ДопСоглашениеОкладДолжность", ОбъектыНазначения.ВыгрузитьЗначения());
...
Процедура Печать(ИдентификаторКоманды, МассивДокументов) Экспорт
	...
	Для Каждого ПараметрыМакета Из МакетИДанныеОбъекта Цикл 
		...
		Если ПолучитьФайлы(ПередаваемыеФайлы,, ИмяВременногоФайла, Ложь) Тогда
			...
			НовыйФайл.УстановитьУниверсальноеВремяИзменения(ШаблонДоговора.ДатаМодификацииУниверсальная);
		...
		Попытка
			...
			ПечатьДокументаWord(ПараметрыМакета, Ворд);			
