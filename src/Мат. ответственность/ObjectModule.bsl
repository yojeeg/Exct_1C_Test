...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'ППФ Материальная ответственность'");
	НоваяКоманда.Идентификатор = "МатОтветственность";
...
Функция ПолучитьДанныеПечати(МассивОбъектов) Экспорт
	...
	ЗапросПоГендиректору = Новый Запрос();
	ЗапросПоГендиректору.Текст  ="ВЫБРАТЬ
	...
	ЗапросПоГендиректору.УстановитьПараметр("Период",ТекущаяДата());								 	
	ВыборкаПоГендиректору = ЗапросПоГендиректору.Выполнить().Выбрать();
	Если ВыборкаПоГендиректору.Следующий() Тогда 
		ФизическиеЛицаЗарплатаКадры.Просклонять(ВыборкаПоГендиректору.Фамилия + " "+ВыборкаПоГендиректору.Имя+" "+ВыборкаПоГендиректору.Отчество, 2, ГенДиректор,ВыборкаПоГендиректору.Пол); 
	КонецЕсли;
	...
	Для Каждого СтрокаПодразделения Из ТаблицаПодразделений Цикл 
		...
		КонецЕсли;
	КонецЦикла;	
	ДолжностьПодразделение = СокрЛП(Строка(ДолжностьВРодПадеже) + " " + ПодразделениеВПадеже);		
	...
	ДобавитьПараметр(СоответствиеПараметров, "ФИОГенДиректора", 		ГенДиректор);
	...
	ДобавитьПараметр(СоответствиеПараметров, "Сумма", 					ДанныеПоСотрудникуИОрганизации.Сумма);
	ДобавитьПараметр(СоответствиеПараметров, "СуммаПрописью", 			РаботаСКурсамиВалют.СформироватьСуммуПрописью(ДанныеПоСотрудникуИОрганизации.Сумма, ЗарплатаКадры.ВалютаУчетаЗаработнойПлаты()));
	...
	ДобавитьПараметр(СоответствиеПараметров, "АдресОрг", 				МассивДанныхЗаполнения[0].ОрганизацияАдресЮридический);
