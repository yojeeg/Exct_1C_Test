...
Функция ПолучитьИтоговуюТаблицу()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
				   ...
				   |	ВЫБОР
				   |	КОГДА ПолныеСведенияНДФЛ.Документ ССЫЛКА Документ.ВедомостьНаВыплатуЗарплатыВБанк
				   |		ТОГДА ВЫРАЗИТЬ(ПолныеСведенияНДФЛ.Документ КАК Документ.ВедомостьНаВыплатуЗарплатыВБанк).Дата
				   |	КОГДА ПолныеСведенияНДФЛ.Документ ССЫЛКА Документ.ВедомостьНаВыплатуЗарплатыВКассу
				   |		ТОГДА ВЫРАЗИТЬ(ПолныеСведенияНДФЛ.Документ КАК Документ.ВедомостьНаВыплатуЗарплатыВКассу).Дата
				   |	ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
				   |	КОНЕЦ КАК ВедомостьДатаВыплаты,
				   ...
				   |	ВЫБОР
				   |		КОГДА ПолныеСведенияНДФЛ.Документ ССЫЛКА Документ.ВедомостьНаВыплатуЗарплатыВБанк
				   |			ТОГДА ПолныеСведенияНДФЛ.Документ.Дата
				   |		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
				   |	КОНЕЦ,         
	               |	ПолныеСведенияНДФЛ.Подразделение,
	               |	ПолныеСведенияНДФЛ.ДокументОснование
	...
	ЗапросОтбор.Текст = "ВЫБРАТЬ
	                    ...
	                    |	ВТДанные.ОКТМО,
	                    |	ВТДанные.КПП,
	                    |	ВТДанные.ФизическоеЛицо.Наименование
	                    |;
	                    |
	                    |////////////////////////////////////////////////////////////////////////////////
	                    |УНИЧТОЖИТЬ ВТДанные
	                    ...
	                    |////////////////////////////////////////////////////////////////////////////////
	                    |ВЫБРАТЬ
	                    |	ВТ_Промежуточная.Период,
	                    |	ВТ_Промежуточная.ДатаВыплаты,
	                    |	ВТ_Промежуточная.Сумма + ВТ_Промежуточная.НДФЛНеудержанный КАК Сумма,
	                    |	ВТ_Промежуточная.НДФЛНеудержанный,
	                    |	ВТ_Промежуточная.ФизическоеЛицо,
	                    |	ВТ_Промежуточная.РегистрацияВНалоговомОргане,
	                    |	ВТ_Промежуточная.Подразделение,
	                    |	ВТ_Промежуточная.ОКТМО,
	                    |	ВТ_Промежуточная.КПП,
	                    |	ВТ_Промежуточная.ФизическоеЛицоНаименование
	                    |ИЗ
