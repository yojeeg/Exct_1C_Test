...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'Доп. соглашение на изменение ТД 01.01.2016'");
	НоваяКоманда.Идентификатор = "ПечатьКомплектаДопСоглашений";
...
Функция ПолучитьДанныеПечати(МассивОбъектов) Экспорт
	...
	АдресОрганизацииКонст = "Российская Федерация, 125047, г. Москва, 4-й Лесной переулок, дом 4";// Отличается от указанного в базе
	...
	Подразделение 	= МассивДанныхЗаполнения[0].Подразделение;
	...
	ДобавитьПараметр(СоответствиеПараметров, "АдресОрг", 				АдресОрганизацииКонст);
	...
	ДобавитьПараметр(СоответствиеПараметров, "Надстрочный1", 			Символ(185));	
	ДобавитьПараметр(СоответствиеПараметров, "Надстрочный2", 			Символ(178));	
...
Функция ПолучитьНадбавку(Сотрудник, ВидНадбавки, Дата = Неопределено)
	...
	ЗапросПоНадбавкам.УстановитьПараметр("Дата",ДатаАктуальности);							  
...
Функция ПолучитьДанныеОДопОтпуске(Сотрудник)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			&Дата,
	               |			Сотрудник = &Сотрудник
	               |				И ВидЕжегодногоОтпуска = &ВидЕжегодногоОтпуска) КАК ПоложенныеВидыЕжегодныхОтпусковСрезПоследних";
	Запрос.УстановитьПараметр("Сотрудник",Сотрудник);				   
