...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'ППФ ТД Менеджер по продажам с РХР и РКС и СН (для регионов)'");
	НоваяКоманда.Идентификатор = "ТД_МенеджерПоПродажамРХРРКССНРегионы";
...
Функция ПолучитьНадбавку(Сотрудник, ВидНадбавки)
	...
	Иначе 
		Если ВидНадбавки.Наименование = "Северная надбавка" Тогда 
			...
			|ИЗ
			|	РегистрСведений.ПроцентыСевернойНадбавкиФизическихЛиц.СрезПоследних(, ФизическоеЛицо = &ФизическоеЛицо) КАК ПроцентыСевернойНадбавкиФизическихЛицСрезПоследних";
