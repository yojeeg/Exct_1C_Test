...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'Доп/ТД Директор по продажам с РХР и РКС и СН (для регионов)'");
	НоваяКоманда.Идентификатор = "Доп_ТД_ДиректорПоПродажамРХРРКССНРегионы";
...
Функция ПолучитьДанныеПечати(МассивОбъектов) Экспорт
	...
	ТаблицаПодразделенийРод = ОпределитьПодразделениеРод(Подразделение);
	...
	Для Каждого СтрокаПодразделения Из ТаблицаПодразделений Цикл 
		...
		КонецЕсли;
	КонецЦикла;		
	Для Каждого СтрокаПодразделения Из ТаблицаПодразделенийРод Цикл 
	...
	КонецЕсли;
	// РК и СН
	РК = ПолучитьНадбавку(Сотрудник, Справочники.ПоказателиРасчетаЗарплаты.РайонныйКоэффициент, ДатаНачРаботы);
	Если РК<>0 Тогда 
		РК = (РК - 1)*100;
	КонецЕсли;
...
Функция ОпределитьПодразделениеРод(Подразделение)
