...
Процедура СформироватьПечатнуюФорму(Команда)
	...
	Печать("ТД_МенеджерПоПродажамРКС", ОбъектыНазначения.ВыгрузитьЗначения());
...
Процедура ЗаменитьТекстВВорде(Ворд, ИсходнаяСтрока, СтрокаЗамены)
	...
	Ворд.Application.Selection.Find.Replacement.Text	= СтрокаЗамены;
...
Функция ПолучитьМакет_Сервер(Идентификатор)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	Файлы.ИмяПредопределенныхДанных
	               |ИЗ
	...
	Если Выборка.Следующий() Тогда 
		ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляСохранения(Выборка.Ссылка, УникальныйИдентификатор);
		Возврат ДанныеФайла;
	Иначе 
