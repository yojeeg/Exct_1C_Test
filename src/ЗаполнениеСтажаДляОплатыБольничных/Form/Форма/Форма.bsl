...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ОбъектыНазначенияМассив) Экспорт
	Сотрудник = ОбъектыНазначенияМассив[0];
	ЭтаФорма.Открыть();
	Инициализация();
КонецПроцедуры
...
Процедура ЗагрузитьИзExcel(Команда)
	...
	СписокКолонок.Добавить("ДатаС", "Дата с");
	СписокКолонок.Добавить("ДатаПо", "Дата по");
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НаименованиеРезультирующейТаблицы","ДатыСтажа");
	ПараметрыФормы.Вставить("СписокКолонок", СписокКолонок);
	
	ОткрытьФорму("ВнешняяОбработка.ЗаполнитьСтажиСотрудника.Форма.ЗагрузкаИзТабличногоДокумента", ПараметрыФормы, ЭтаФорма);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ДатыСтажа" Тогда		
		ЗаполнитьТаблицуПериодов(Параметр);   
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТаблицуПериодов(АдресВХранилищеЗначений)	
	...
	Если ТипЗнч(ТабЗнч) = Тип("ТаблицаЗначений") Тогда
		...
		Для Каждого СтрокаТаблицы Из ТабЗнч Цикл 
			НоваяСтрока = ТрудоваяДеятельностьФизическихЛиц.Добавить();
			НоваяСтрока.ФизическоеЛицо 	= ФизическоеЛицоСсылка;
			НоваяСтрока.ДатаНачала 		= Дата(Прав(СтрокаТаблицы.ДатаС,4) 	+ Сред(СтрокаТаблицы.ДатаС, 4,2) 	+ Лев(СтрокаТаблицы.ДатаС, 2));
			НоваяСтрока.ДатаОкончания 	= Дата(Прав(СтрокаТаблицы.ДатаПо,4) + Сред(СтрокаТаблицы.ДатаПо, 4,2) 	+ Лев(СтрокаТаблицы.ДатаПо, 2));
