...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'Доп.соглашение изменение адреса'");
	НоваяКоманда.Идентификатор = "СпискомДопСоглашениеИзменениеАдреса";
...
Функция ПолучитьДанныеПечати(МассивОбъектов) Экспорт
	...
	СоответствиеПараметров = СтруктураПечати();
	...
	Если ЗначениеЗаполнено(Сотрудник) Тогда 
		...
		ДанныеПоСотрудникуИОрганизации = СформироватьДанныеПоСотрудникуИОрганизации(Сотрудник, ФизЛицо);
		...
		ДобавитьПараметр(СоответствиеПараметров, "ДатаТД", 					МассивДанныхЗаполнения[0].ТрудовойДоговорДата);
		ДобавитьПараметр(СоответствиеПараметров, "НомерТД", 				МассивДанныхЗаполнения[0].ТрудовойДоговорНомер);
		ДобавитьПараметр(СоответствиеПараметров, "ДатаДопСогл", 			Формат(ДатаДопСогл,"ДЛФ=ДД"));
...
Функция ПолучитьДанныеДляПечати(Сотрудник)
	...
	Если ЗначениеЗаполнено(Сотрудник) Тогда 
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ПриемНаРаботу.ТрудовойДоговорНомер КАК НомерТД,
		               |	ПриемНаРаботу.ТрудовойДоговорДата КАК ДатаТД,
		               |	ПОДСТРОКА(ЕСТЬNULL(ФИОФизическихЛицСрезПоследних.Имя, """"), 1, 1) + ""."" + ПОДСТРОКА(ЕСТЬNULL(ФИОФизическихЛицСрезПоследних.Отчество, """"), 1, 1) + "". "" + ЕСТЬNULL(ФИОФизическихЛицСрезПоследних.Фамилия, """") КАК ИОФ,
		               ...
		               |		ПО ТекущиеКадровыеДанныеСотрудников.ГоловнаяОрганизация = Организации.Ссылка
		               |			И ТекущиеКадровыеДанныеСотрудников.ТекущаяОрганизация = Организации.Ссылка
		               |			И ТекущиеКадровыеДанныеСотрудников.УдалитьТекущаяОрганизация = Организации.Ссылка
		               |ГДЕ
...
Функция ДанныеДляПечатиТрудовогоДоговора(МассивОбъектов)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не Результат.Пустой() Тогда 
			Выборка = Результат.Выбрать();
			Если Выборка.Следующий() Тогда 
				ПараметрыТрудовогоДоговора.ФамилияИО = Лев(Выборка.Имя,1) +"."+ Лев(Выборка.Отчество,1)+". "+ Выборка.Фамилия;
				ПараметрыТрудовогоДоговора.ФИОПолные = Выборка.Фамилия + " " + Выборка.Имя + " " + Выборка.Отчество;
			КонецЕсли;
		КонецЕсли; 
		
		// Получим актуальные данные по должности, подразделению
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		...
		Если Не Результат.Пустой() Тогда 
			Выборка = Результат.Выбрать();
			Если Выборка.Следующий() Тогда 
				ПараметрыТрудовогоДоговора.Подразделение 	= Выборка.Подразделение;
				ПараметрыТрудовогоДоговора.Должность 		= Выборка.Должность;
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьДанныеПоСотрудникуИОрганизации(Сотрудник, ФизЛицо)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЗначенияПериодическихПоказателейРасчетаЗарплатыСотрудников.СрезПоследних(&Дата, Показатель = &Показатель) КАК ЗначенияПериодическихПоказателейРасчетаЗарплатыСотрудниковСрезПоследних
...
Функция СтруктураПечати()
	...
	СтруктураПечати.Вставить("НомерТД");
	СтруктураПечати.Вставить("ДатаТД");
	СтруктураПечати.Вставить("ДатаДопСогл");
	СтруктураПечати.Вставить("Организация");
	СтруктураПечати.Вставить("ФИО");
	СтруктураПечати.Вставить("НомерТД");
	СтруктураПечати.Вставить("ДатаТД");
	СтруктураПечати.Вставить("ОрганизацияСокр");
	СтруктураПечати.Вставить("АдресОрг");
	СтруктураПечати.Вставить("ТелефонОрг");
	СтруктураПечати.Вставить("ИННОрг");
	СтруктураПечати.Вставить("БИКОрг");
	СтруктураПечати.Вставить("РасчСчет");
	СтруктураПечати.Вставить("НаимБанка");
	СтруктураПечати.Вставить("КорСчет");
	СтруктураПечати.Вставить("ТабНомер");
	СтруктураПечати.Вставить("ДатаРождения");
	СтруктураПечати.Вставить("Паспорт");
	СтруктураПечати.Вставить("ДатаВыдачиПаспорта");
	СтруктураПечати.Вставить("АдресРегистрации");
	СтруктураПечати.Вставить("ИНН");
	СтруктураПечати.Вставить("СтрахСвид");
	СтруктураПечати.Вставить("ДомТелефон");
	СтруктураПечати.Вставить("ИОФ");
