Функция СведенияОВнешнейОбработке() Экспорт
   ...
   РегистрационныеДанные.Вставить("Наименование", "Выгрузка данных по сотрудникам в бухгалтерию");
   РегистрационныеДанные.Вставить("БезопасныйРежим", Ложь);
   РегистрационныеДанные.Вставить("Версия", "1.0");
   РегистрационныеДанные.Вставить("Вид", "ДополнительнаяОбработка");
   РегистрационныеДанные.Вставить("Информация", "Выгрузка данных по сотрудникам в бухгалтерию");
   ...
   Команда.Представление = "Выгрузка данных по сотрудникам в бухгалтерию";
...
Процедура ВыполнитьКоманду(ИдентификаторКоманды = Неопределено) Экспорт
	...
	ПараметрыПодключения = Новый Структура("Версия,Сервер,ИмяБазаДанных,Пользователь,Пароль","V83","RU00SP1C01","RU10","Синхронизация", "7NtSydTYtxTsnBChhBLq");
	...
	Если COM_Соединение <> Неопределено Тогда
		...
		|ГДЕ
		|	ТекущиеКадровыеДанныеСотрудников.ТекущийВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ТрудовойДоговор)
		...
		|	И ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения = ДАТАВРЕМЯ(1, 1, 1)
		|	И ТекущиеКадровыеДанныеСотрудников.ТекущийВидДоговора = ЗНАЧЕНИЕ(Перечисление.ВидыДоговоровССотрудниками.ТрудовойДоговор)
		...
		Если ФизЛица.Количество() > 0 Тогда
			...
			|	ТЗ_ФИЗ_ЛИЦА.ИНН,
			|	ТЗ_ФИЗ_ЛИЦА.Наименование,
			|	ТЗ_ФИЗ_ЛИЦА.ПометкаУдаления,
			|	ТЗ_ФИЗ_ЛИЦА.ДатаРождения,
			|	ТЗ_ФИЗ_ЛИЦА.Пол
			...
			|	ВТ_ФИЗ_ЛИЦА.ИНН,
			|	ВТ_ФИЗ_ЛИЦА.Наименование,
			|	ВТ_ФИЗ_ЛИЦА.ПометкаУдаления,
			|	ВТ_ФИЗ_ЛИЦА.ДатаРождения,
			|	ВТ_ФИЗ_ЛИЦА.Пол
			|ИЗ
			|	ВТ_ФИЗ_ЛИЦА КАК ВТ_ФИЗ_ЛИЦА
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
			|		ПО ВТ_ФИЗ_ЛИЦА.ФизЛицоКод = ВЫРАЗИТЬ(ФизическиеЛица.Код КАК СТРОКА(10))
			|ГДЕ
			|	ВТ_ФИЗ_ЛИЦА.ФизЛицоКод <> """"
			|	И (ФизическиеЛица.ЭтоГруппа = ЛОЖЬ ИЛИ ФизическиеЛица.ЭтоГруппа ЕСТЬ NULL)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			...
			Если Не Результат6.Пустой() Тогда
				...
				Пока Выборка6.Следующий() Цикл
					Если COM_Соединение.ЗначениеЗаполнено(Выборка6.Ссылка) Тогда	
						
						ОбъектСотрудник = Выборка6.Ссылка.ПолучитьОбъект();
					...
					Иначе
						...
						НовыйСотрудник.ИНН 				= Выборка6.ИНН;
						НовыйСотрудник.Наименование 	= Выборка6.Наименование;
						НовыйСотрудник.ПометкаУдаления 	= Выборка6.ПометкаУдаления;
						НовыйСотрудник.Код 				= Выборка6.ФизЛицоКод;
						НовыйСотрудник.Пол 				= Выборка6.Пол;
						НовыйСотрудник.ФИО 				= Выборка6.Наименование;
		...
		Если ПаспортныеДанные.Количество() > 0 Тогда
			...
			|	ВТ_ПД КАК ВТ_ПД
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
			|		ПО ВТ_ПД.ФизЛицоКод = ФизическиеЛица.Код
			|ГДЕ
			...
			Если Не Результат7.Пустой() Тогда
				...
				Пока Выборка7.Следующий() Цикл
					...
					Если COM_Соединение.ЗначениеЗаполнено(ВидДокумента) Тогда
		...
		Если КонтактнаяИнформация.Количество() > 0 Тогда
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
			|		ПО ВТ_КОНТАКТНАЯ_ИНФОРМАЦИЯ.ФизЛицоКод = ФизическиеЛица.Код
			|ГДЕ
			|	НЕ ФизическиеЛица.Ссылка ЕСТЬ NULL
			...
			Если Не Результат9.Пустой() Тогда
				...
				Пока ВыборкаФизЛицо.Следующий() Цикл			
					Если COM_Соединение.ЗначениеЗаполнено(ВыборкаФизЛицо.Ссылка) Тогда
						Попытка
		...
		Если Не Результат10.Пустой() Тогда
			...
			Пока Выборка10.Следующий() Цикл
				...
				Контрагент.ФизическоеЛицо 					= Выборка10.ФизЛицо;
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
		|		ПО ВТ_ЛИЦСЧЕТА.КодФизЛица = ФизическиеЛица.Код
		|ГДЕ
		|	НЕ ФизическиеЛица.Ссылка ЕСТЬ NULL 
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|		ПО ВТ_ФИЗ_ЛИЦА_ЛИЦ_СЧЕТА.Ссылка = Контрагенты.ФизическоеЛицо
		|ГДЕ
		...
		|	ВТ_ФИО КАК ВТ_ФИО
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ФизическиеЛица КАК ФизическиеЛица
		|		ПО ВТ_ФИО.ФизЛицоКод = ФизическиеЛица.Код
		|ГДЕ
		|	НЕ ФизическиеЛица.Ссылка ЕСТЬ NULL
		|   И ВТ_ФИО.ФизЛицоКод <> """"
		|;
