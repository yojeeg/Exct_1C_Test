...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'ППФ Доп.соглашение адрес агентства'");
	НоваяКоманда.Идентификатор = "ДопСоглашениеАдрес";
...
Функция ПолучитьДанныеПечати(МассивОбъектов) Экспорт
	...
	КонецЕсли;
	МассивДанныхЗаполнения = ДанныеДляПечатиТрудовогоДоговора(Сотрудник);
	...
	Надбавки 		= "";
	//ДатаДоговора 	= МассивДанныхЗаполнения[0].Ссылка.ДатаНачала;
	ДатаНачРаботы 	= МассивДанныхЗаполнения[0].ТрудовойДоговорДата;
	Подразделение 	= ПолучитьПодразделениеСотрудника(Сотрудник);
	...
	Для Каждого СтрокаПодразделения Из ТаблицаПодразделений Цикл 
		...
		КонецЕсли;
	КонецЦикла;	
	ДолжностьПодразделение = СокрЛП("" + ДолжностьВРодПадеже + " " + ПодразделениеВПадеже);		
	РайКоэ = ПолучитьРайКоэф(Сотрудник, ТекущаяДата());
	СерКоэ = Сотрудник.ТекущийПроцентСевернойНадбавки;
	ДопОтп = ПолучитьДопОтпуск(Сотрудник, ТекущаяДата());
	ДанныеПоСотрудникуИОрганизации = СформироватьДанныеПоСотрудникуИОрганизации(Сотрудник, ФизЛицо, ТекущаяДата());
	...
	ДобавитьПараметр(СоответствиеПараметров, "ДатаТД_1", 				Формат(ТекущаяДата(),"ДЛФ=DD"));
	ДобавитьПараметр(СоответствиеПараметров, "НомерТД", 				МассивДанныхЗаполнения[0].ТрудовойДоговорНомер);
	ДобавитьПараметр(СоответствиеПараметров, "ФИО", 					СокрЛП(ФИО));
	...
	ДобавитьПараметр(СоответствиеПараметров, "Сумма", 					Формат(МассивДанныхЗаполнения[0].Ссылка.СовокупнаяТарифнаяСтавка));
	ДобавитьПараметр(СоответствиеПараметров, "СуммаПрописью", 			ЧислоПрописью(МассивДанныхЗаполнения[0].Ссылка.СовокупнаяТарифнаяСтавка, "НП=Ложь",",,,,,,,,0"));
	...
	ДобавитьПараметр(СоответствиеПараметров, "ДатаСДопСогл", 			Формат(МассивДанныхЗаполнения[0].Ссылка.ДатаПриема, "ДЛФ=ДД"));
	ДобавитьПараметр(СоответствиеПараметров, "ДатаДопСогл", 			Формат(МассивДанныхЗаполнения[0].Ссылка.ДатаПриема, "ДЛФ=ДД"));
	ДобавитьПараметр(СоответствиеПараметров, "НомерДопСогл", 			МассивДанныхЗаполнения[0].Ссылка.Комментарий);
	ДобавитьПараметр(СоответствиеПараметров, "Подчиняется", 			"начальнику отдела");
	ДобавитьПараметр(СоответствиеПараметров, "РКСКоэффициент", 			?(ПустаяСтрока(РайКоэ), "0", РайКоэ));
	ДобавитьПараметр(СоответствиеПараметров, "РКСНадбавка", 			?(ПустаяСтрока(СерКоэ), "0", СерКоэ));	
	ДобавитьПараметр(СоответствиеПараметров, "РКСОтпуск", 				?(СокрЛП(ДопОтп) <> "",СокрЛП(ДопОтп),"0"));	
	ДобавитьПараметр(СоответствиеПараметров, "ДатаТДСокр", 				Формат(ДатаНачРаботы,"ДФ=dd.MM.yyyy"));	
	ДобавитьПараметр(СоответствиеПараметров, "ДатаПеревода", 			Формат(ТекущаяДата(), "ДЛФ=ДД"));	
...
Функция ДанныеДляПечатиТрудовогоДоговора(МассивОбъектов)
	...
	ЗапросПоПриемамНаРаботу.УстановитьПараметр("Сотрудник",МассивОбъектов);								   
