...
Функция ПолучитьДанныеПечати(МассивОбъектов) Экспорт
	...
	ЗапросФИО.УстановитьПараметр("ФизическоеЛицо",ДанныеПоСотруднику[0].ФизическоеЛицо);
	...
	КонецЕсли;
	Надбавки = "";
	ДатаДоговора = ДанныеПоСотруднику[0].ДатаДок;
	ДатаНачРаботы = ДатаПриема;	
	ТаблицаПрошлоеПодразделение = ОпределитьПодразделение(ДанныеПоСотруднику[0].Подразделение);
	ТаблицаПрошлаяДолжность 	= ОпределитьДолжность(ДанныеПоСотруднику[0].Должность);
	ТаблицаНовоеПодразделение 	= ОпределитьПодразделение(ДанныеПоСотруднику[0].НовоеПодразделение);
	ТаблицаНоваяДолжность 		= ОпределитьДолжность(ДанныеПоСотруднику[0].НоваяДолжность);
	...
	Оклад=ДанныеПоСотруднику[0].ТарифнаяСтавка;
	//Для Каждого СтрНачисл Из МассивОбъектов[0].Начисления Цикл
	//	Если СтрНачисл.Начисление=ПланыВидовРасчета.Начисления.НайтиПоНаименованию("Оплата по окладу") Тогда
	//		Оклад=СтрНачисл.Размер;
	//		Прервать
	//	КонецЕсли;
	//КонецЦикла;
	...
	ДобавитьПараметр(СоответствиеПараметров, "НомерТД", 				ДанныеПоСотруднику[0].НомерДок);
	ДобавитьПараметр(СоответствиеПараметров, "ФИО", 					ПросклонятьФИО(СокрЛП(ФИО), 2, ДанныеПоСотруднику[0].НовыйПол));		
	...
	ДобавитьПараметр(СоответствиеПараметров, "Сумма", 					Оклад);
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'ППФ Заявление на перевод к Т5'");
