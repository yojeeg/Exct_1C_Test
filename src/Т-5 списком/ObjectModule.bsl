...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование", " Приказ о переводе (Т-5) списком"); //имя под которым обработка будет зарегестрирована в справочнике внешних обработок
	ПараметрыРегистрации.Вставить("Версия", "1.0");
	ПараметрыРегистрации.Вставить("БезопасныйРежим", Ложь);
	ПараметрыРегистрации.Вставить("Информация", " Приказ о переводе (Т-5) списком");//так будет выглядеть описание печ.формы для пользователя
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	ДобавитьКоманду(ТаблицаКоманд, "ППФ Приказ о переводе (Т-5) списком", "ПФ_MXL_Т5", "ВызовСерверногоМетода", Истина);
...
Функция ПолучитьДанныеДляПечатиКадровогоПриказаТ5(МассивОбъектов)
	...
	Пока  Выборка.Следующий() Цикл
		...
		НомерПробела = СтрНайти(Выборка.НомерДок," ");
		Если НомерПробела<>0 Тогда 
			НомерДокумента = Лев(Выборка.НомерДок,НомерПробела-1)+"\"+сч+" ";
			Параметры.НомерДок = НомерДокумента + Прав(Выборка.НомерДок,СтрДлина(Выборка.НомерДок)-НомерПробела);
		КонецЕсли;
